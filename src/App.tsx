...
export default function App() {
  const [details, setDetails] = useState<TenancyDetails>(() => {
    const saved = localStorage.getItem('ast_draft_details');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('ast_draft_details', JSON.stringify(details));
  }, [details]);
